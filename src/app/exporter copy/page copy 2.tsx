...
const App: React.FC = () => {
  ...
  const [trimStart, setTrimStart] = useState<number>(3); // Start time for trimming
  const [trimDuration, setTrimDuration] = useState<number>(2); // Duration of trim
  ...
  const trimVideo = async () => {
    ...
    return 'trimmed.mp4'; // Return the path to the trimmed video
  };

  const processFrames = async (trimmedVideoPath: string) => {
    ...
    // Use the trimmed video instead of the original
    video.src = URL.createObjectURL(
      new Blob([trimmedVideoPath], { type: 'video/mp4' })
    );
    await video.load(); // Ensure the video is loaded
  ...
  const handleExport = async () => {
    setIsProcessing(true);
    // Trim video before processing frames
    const trimmedVideoPath = await trimVideo();
    setIsProcessing(false);

    // After trimming, load the video and process the frames
    if (trimmedVideoPath) {
      const frames = await processFrames(trimmedVideoPath); // Pass trimmed video path to processFrames
      if (frames) {
        // Now call exportVideo after frames are processed
