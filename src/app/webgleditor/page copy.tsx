...
const fragmentShaderSource = `
  ...
  void main() {
    ...
    if (v_texCoord.x > u_rectangle.x && v_texCoord.x < u_rectangle.x + u_rectangle.z &&
        v_texCoord.y > u_rectangle.y && v_texCoord.y < u_rectangle.y + u_rectangle.w) {
      color.rgb = vec3(1.0, 0.0, 0.0); // Red color for rectangle
    }
...
const App: React.FC = () => {
  ...
  const handleBoxSelection = (width: number, height: number) => {
    setSelectedBox({ width, height });
    setRect({
      x: 10, // Start at 10px, or could be adjusted based on the mouse position
      y: 10,
      width,
      height,
    });
  ...
  const processVideoFrames: any = async (exportFrames: boolean = false) => {
    ...
    const rectangleLocation = gl.getUniformLocation(program, 'u_rectangle');
  ...
  return (
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ marginRight: '20px' }}>
        <h3>Select a Box</h3>
        <button onClick={() => handleBoxSelection(100, 50)}>
          Box 1 (100x50)
        </button>
        <button onClick={() => handleBoxSelection(150, 80)}>
          Box 2 (150x80)
        </button>
        <button onClick={() => handleBoxSelection(200, 100)}>
          Box 3 (200x100)
        </button>
      </div>
